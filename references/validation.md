...
最后逐页校对标题、正文、脚注和演讲者备注：检查语病、错别字、数字单位与重复指标。成稿备注不保留“配图必须替换”“后续由总编统一”等未完成的制作指令；版式变动后删除或改正不再存在的头像、徽章、箭头或行列描述，同时保留事实、来源、日期、统计口径与限制条件。修正文案后重新核对受影响页的实际换行和溢出。
...
飞书回读可能把连续同宽列合并为 `<col span="3" width="240"/>`，它表示三列、总宽 720，并非单列。布局检查和近似预览按逻辑列数展开；`span` 应为有限的正整数列数，接受 `3.0` 等等值数值形式。不要因回读省略了重复 `<col>` 就改动正确的表格。

...
准备工具要求未完成字段被处理后才生成字段完整稿；草稿输出必须显式带 allow-draft。字段完整稿仍需按实际内容调整版式。为避免覆盖编辑中的文件，将其输出到新的目录：
...
- 绑定只替换字段，不自动重设图表轴范围、柱宽、单位或数值格式。例：模板的两图轴上限为 40/80，正文改为“共用 0–35 刻度”后，必须同步修改两图的轴范围；不能只确认数据值正确。按实际数据检查零基线、比较条件、标签和相邻摘要数字。
- 换图后按真实宽高比调整图片框并检查占比、裁剪与相邻文字位置；占位图的横向比例不应直接用于更高的产品截图。图表和图片布局修改后重新运行检查并获取实际截图。
